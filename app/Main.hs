...
drawMain gs = [C.center $ W.hBox [
            B.border $ W.padLeftRight 3 $ W.padTopBottom 1 $ boardWidget (gs ^. board),
            B.border $ W.padLeftRight 5 $ W.padTopBottom 5 $ menuWidget (gs ^. menu)
                                 ]]

handleMain :: GameState -> BrickEvent n e -> EventM n (Next GameState)
handleMain gs (VtyEvent (V.EvKey (V.KChar 'c') [V.MCtrl])) = halt gs
handleMain gs@(GS _ _ _ (Just d)) e = continue $ handleDialog e gs
...
handleMain gs e = case gs ^. focus of
                  ...
                  OnMenu    -> continue $ handleMenu e gs
